...
func RegisterPostServiceHTTPServer(s *http.Server, srv PostServiceHTTPServer) {
	r := s.Route("/")
	r.POST("/v1/billing/post/list", _PostService_ListPost0_HTTP_Handler(srv))
	r.GET("/v1/billing/posts", _PostService_ListPost1_HTTP_Handler(srv))
	r.GET("/v1/billing/post/{id}", _PostService_GetPost0_HTTP_Handler(srv))
	r.POST("/v1/billing/post", _PostService_CreatePost0_HTTP_Handler(srv))
	r.PATCH("/v1/billing/post/{post.id}", _PostService_UpdatePost0_HTTP_Handler(srv))
	r.PUT("/v1/billing/post/{post.id}", _PostService_UpdatePost1_HTTP_Handler(srv))
	r.DELETE("/v1/billing/post/{id}", _PostService_DeletePost0_HTTP_Handler(srv))
}
...
func (c *PostServiceHTTPClientImpl) UpdatePost(ctx context.Context, in *UpdatePostRequest, opts ...http.CallOption) (*Post, error) {
	var out Post
	pattern := "/v1/billing/post/{post.id}"
	path := binding.EncodeURL(pattern, in, false)
